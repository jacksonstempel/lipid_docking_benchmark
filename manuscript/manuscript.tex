...
Despite advances in structural biology, experimentally resolving lipid binding poses and distinguishing functionally relevant bound lipids from adventitious ones remain difficult, motivating computational docking methods.\cite{hunte2008lipids}
...
Their hydrocarbon tails are highly flexible, while the biologically relevant determinants of binding are often concentrated in the polar headgroup, which forms specific hydrogen-bonding and electrostatic contacts; the tail, by contrast, contributes largely hydrophobic packing.
...
Long flexible tails and the biological importance of headgroup placement make lipids a distinct and demanding target for conventional docking scoring functions.
...
Prior to each GNINA run, the prepared ligand PDBQT was converted to SDF with OpenBabel inside the container to preserve explicit bonding, and outputs were written back to PDBQT for compatibility with the benchmark pipeline.
...
Figure~\ref{fig:structural_examples} illustrates a characteristic Vina failure mode: on highly flexible lipids the top-ranked pose can collapse into a visibly tangled non-native conformation (Panel A). A second pattern observed during case review is inversion of the headgroup and tail within the correct pocket, which yields deceptively plausible pocket-level overlap despite poor RMSD.
...
This sensitivity to local chemistry suggests that Boltz-2 has learned aspects of protein--lipid interaction physics, rather than merely recalling where lipids typically sit in familiar protein folds.
...
For lipid systems where headgroup chemistry and local protein interactions dominate specificity, jointly generated protein--ligand models performed especially well in the setting evaluated here.
...
The benchmark dataset, analysis code, manuscript source, and archived benchmark-result bundle are publicly available to facilitate further method development.
	...
	The benchmark dataset, analysis code, manuscript source, and benchmark-result archive are available at \url{https://github.com/jacksonstempel/lipid_docking_benchmark}.
