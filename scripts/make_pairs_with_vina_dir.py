#!/usr/bin/env python3
"""
Create a new pairs CSV using an alternate Vina prediction directory.

The benchmark reads a "pairs CSV" (structures/benchmark_entries.csv by default)
with columns: pdbid, ref, boltz_pred, vina_pred.

This script rewrites only the `vina_pred` column to point at a new directory
(e.g., structures/vina_isaac/) while keeping `ref` and `boltz_pred` identical.
"""
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--out-pairs",
        type=Path,
        default=Path("structures/benchmark_entries_vina_isaac.csv"),
        help="Output pairs CSV (default: structures/benchmark_entries_vina_isaac.csv).",
    )
    parser.add_argument(
        "--vina-dir",
        type=Path,
        default=Path("structures/vina_isaac"),
        help="Directory containing Vina PDBQTs named <PDBID>.pdbqt (default: structures/vina_isaac).",
    )
    parser.add_argument(
        "--suffix",
        default=".pdbqt",
        help="Filename suffix appended to PDBID inside --vina-dir (default: .pdbqt).",
    )
    ...
    vina_dir = (project_root / args.vina_dir).resolve() if not args.vina_dir.is_absolute() else args.vina_dir.resolve()

    rows = _read_pairs(in_pairs)

    vina_dir_rel = vina_dir.relative_to(project_root).as_posix().rstrip("/")
    missing: List[str] = []
    out_rows: List[Dict[str, str]] = []
    for row in rows:
        pdbid = row["pdbid"].strip().upper()
        vina_rel = f"{vina_dir_rel}/{pdbid}{args.suffix}"
        out_row = dict(row)
        out_row["pdbid"] = pdbid
        out_row["vina_pred"] = vina_rel
        out_rows.append(out_row)

        if not (project_root / vina_rel).exists():
            missing.append(vina_rel)

    if missing:
        raise SystemExit(
            "Missing Vina predictions (first 10 shown):\n"
            + "\n".join(missing[:10])
        )
