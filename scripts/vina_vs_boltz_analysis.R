...
packages <- c("ggplot2", "dplyr", "readr", "tidyr", "scales", "gridExtra", "broom")
for (pkg in packages) {
...
df <- inner_join(
  boltz %>% select(pdbid, rmsd_locked_global_boltz = rmsd_locked_global),
...
df <- df %>%
  mutate(
    diff = rmsd_locked_global_vina - rmsd_locked_global_boltz,
...
summary_tbl <- tibble(
  method = c("Boltz", "Vina"),
  mean = c(mean(df$rmsd_locked_global_boltz), mean(df$rmsd_locked_global_vina)),
  median = c(median(df$rmsd_locked_global_boltz), median(df$rmsd_locked_global_vina)),
  sd = c(sd(df$rmsd_locked_global_boltz), sd(df$rmsd_locked_global_vina)),
...
stats_out <- list(
  ...
  summary = summary_tbl,
  success = success_tbl,
  paired_t = tidy(paired_t),
  wilcoxon = tidy(wilcox),
...
write_csv(tidy(paired_t), file.path(out_dir, "paired_t_test.csv"))
write_csv(tidy(wilcox), file.path(out_dir, "wilcoxon_signed_rank.csv"))
write_csv(tibble(metric = c("Vina_not_worse", "Boltz_better"), value = c(sign_wins_vina, sign_wins_boltz)),
          file.path(out_dir, "sign_test_counts.csv"))

palette_cols <- c("#4C72B0", "#DD8452")

p_scatter <- ggplot(df, aes(x = rmsd_locked_global_boltz, y = rmsd_locked_global_vina)) +
  geom_point(alpha = 0.75, color = palette_cols[1], size = 2.2) +
...
p_hist <- ggplot(df, aes(x = diff)) +
  geom_histogram(bins = 20, fill = palette_cols[2], color = "white", alpha = 0.9) +
  geom_vline(xintercept = mean(df$diff), color = palette_cols[1], linetype = "dashed") +
...
df_long <- df %>%
  ...
  pivot_longer(cols = c(Boltz, Vina), names_to = "method", values_to = "rmsd")
...
succ_long <- success_tbl %>%
  pivot_longer(cols = c(Boltz, Vina), names_to = "method", values_to = "successes")
...
scatter_path <- file.path(out_dir, "scatter.png")
hist_path    <- file.path(out_dir, "diff_histogram.png")
box_path     <- file.path(out_dir, "boxplot.png")
bar_path     <- file.path(out_dir, "success_bar.png")
panel_path   <- file.path(out_dir, "summary_panel.png")

ggsave(scatter_path, p_scatter, width = 6, height = 6, dpi = 300)
ss <- 6

ggsave(hist_path, p_hist, width = 6, height = 4.5, dpi = 300)

ggsave(box_path, p_box, width = 4.5, height = 4.5, dpi = 300)

ggsave(bar_path, p_bar, width = 5.5, height = 4.5, dpi = 300)

panel <- grid.arrange(p_scatter, p_hist, p_box, p_bar, ncol = 2)

ggsave(panel_path, panel, width = 10, height = 8, dpi = 300)
