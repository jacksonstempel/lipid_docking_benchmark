...
def main() -> int:
    ...
    logging.info("Running RMSD batch (Boltz)...")
    _run_cmd(
        [
            "python",
    ...
    logging.info("Running RMSD batch (Vina, %d poses)...", VINA_MAX_POSES)
    _run_cmd(
        [
            "python",
            str(project_root / "scripts" / "measure_ligand_pose_batch.py"),
    ...
    # Contact extraction
    logging.info("Extracting contacts (refs/Boltz/Vina)...")
    ...
    logging.info("Computing contact metrics...")
    ...
    logging.info("Done.")
    logging.info("Outputs: per-pose %s, summary %s", per_pose_path, summary_path)
    return 0
